# Give Test XP to Agent #01 (Debug Version)
...
$body = @{
    agent_id = "ea092228-1ed0-430c-8689-ba199566d220"
    event_type = "XP_GAIN"
    payload = @{
        pontos = 100
        motivo = "Teste de Ativacao"
    }
...
try {
    Invoke-WebRequest -Uri $url -Method Post -Body $body -Headers $headers -ContentType "application/json"
    Write-Host "`nSucesso!"
} catch {
    Write-Host "Erro 400 Detalhado:"
